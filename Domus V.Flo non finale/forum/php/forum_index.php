...
$query=$db->prepare('SELECT cat_id, cat_name, 
	forum_forum.forum_id, forum_name, forum_desc, forum_post, forum_topic, auth_view, forum_topic.topic_id, forum_topic.topic_post, post_id, post_time, post_createur, uidUsers, 
	idUsers 
	FROM forum_category
	LEFT JOIN forum_forum ON forum_category.cat_id = forum_forum.forum_cat_id
	LEFT JOIN forum_post ON forum_post.post_id = forum_forum.forum_last_post_id
	LEFT JOIN forum_topic ON forum_topic.topic_id = forum_post.topic_id
	LEFT JOIN users ON users.idUsers = forum_post.post_createur
	WHERE auth_view <= :lvl 
...
while($data = $query->fetch())
{
    ...
    <a href="./forum_display.php?f='.$data['forum_id'].'">
    ...
    if (!empty($data['forum_post']))
    {
	 ...
	 $page = ceil($nbr_post / $nombreDeMessagesParPage);
         ...
         <a href="./voirprofil.php?m='.stripslashes(htmlspecialchars($data['idUsers'])).'&amp;action=consulter">'.$data['uidUsers'].'  </a>
